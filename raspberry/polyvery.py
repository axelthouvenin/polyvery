...
def LirePortSerie():
    while 1:
        ...
        if "angle" in str_trame :
            pos = str_trame.find("angle:")+6
            pos2 = str_trame.find("\r")-4
            if "-" in str_trame[pos:pos2]:
                angle = float(str_trame[pos+1:pos2]) *-1
            else:
                angle = float(str_trame[pos:pos2])
        else:
            angle = 0
        if "FM=1" in str_trame :
            EviterObstacleFrontal(angle)
            
# Fonction de lecture de l'angle de la boussole
def LireAngle():
    str_trame =str(ser.readline())
    if "angle" in str_trame :
        pos = str_trame.find("angle:")+6
        pos2 = str_trame.find("\r")-4
        if "-" in str_trame[pos:pos2]:
...
thread_serie = threading.Thread(target = LirePortSerie)
